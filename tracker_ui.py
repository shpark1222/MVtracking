...
class ValveTracker(QtWidgets.QMainWindow):
    ...
    def run_sanity_checks(self):
        ...
        if self.active_cine_key in self.pack.cine_planes:
            ...
            H_raw, W_raw = self._get_cine_frame_raw(self.active_cine_key, 0).shape
            corners = np.array(
                [
                    [0.0, 0.0],
                    [W_raw - 1.0, 0.0],
            ...
            corner_xyz = cine_line_to_patient_xyz(corners, cine_geom)
            corner_vox = self._patient_to_voxel(corner_xyz)
            self._log(
                f"{self.active_cine_key} corners voxel coords: "
                f"{np.array2string(corner_vox, precision=3, separator=',')}"
            )
        ...
        if self.active_cine_key in self.pack.cine_planes:
            ...
            nz_frac = float(np.mean(Ipcm > 0))
            p95 = float(np.percentile(Ipcm, 95.0)) if np.any(Ipcm > 0) else 0.0
            self._log(f"reslice Ipcm nonzero fraction: {nz_frac:.4f}, p95: {p95:.6f}")
