...
class ValveTracker(QtWidgets.QMainWindow):
    ...
    def show_plane_overlay(self):
        ...
        for cine_key in cine_keys:
            ...
            roi = self.cine_line_rois.get(cine_key)
            if roi is not None:
                line_xy = self._roi_points_abs(roi)
                line_xy = self._canonicalize_line_abs(line_xy)
            else:
                if self.line_norm[t] is None:
                    self.line_norm[t] = self._default_line_norm()
                display_img = self._get_cine_frame(cine_key, t)
                line_xy = self._norm_to_abs_line(self.line_norm[t], display_img.shape[0], display_img.shape[1])
            line_xy = self._inverse_transform_line_from_display(line_xy, H_raw, W_raw)
    ...
    def on_cine_line_changed_live(self, cine_key: str):
        ...
        img = self._get_cine_frame(cine_key, t)
        H, W = img.shape
    ...
    def set_phase(self, t: int):
        ...
        try:
            ...
            H, W = img.shape
            pts_abs = self._norm_to_abs_line(self.line_norm[t], H, W)
            self._apply_line_abs_to_roi(self.cine_line_rois[self.active_cine_key], pts_abs, H, W)
        finally:
            self._syncing_cine_line = False
    ...
    # Reslice
    # ============================
    def _get_active_line_abs(self, t: int) -> np.ndarray:
        img = self._get_cine_frame(self.active_cine_key, t)
        H, W = img.shape
        if self.line_norm[t] is None:
            self.line_norm[t] = self._default_line_norm()
        return self._norm_to_abs_line(self.line_norm[t], H, W)

    def reslice_for_phase(self, t: int):
        line_xy = self._get_active_line_abs(t)
        cine_img_raw = self._get_cine_frame_raw(self.active_cine_key, t)
        line_xy = self._inverse_transform_line_from_display(
            line_xy,
            cine_img_raw.shape[0],
            cine_img_raw.shape[1],
        )
        pcmra3d = self.pack.pcmra[:, :, :, t].astype(np.float32)
