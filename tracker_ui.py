...
class ValveTracker(QtWidgets.QMainWindow):
    def __init__(
        ...
        self.btn_sanity_check = QtWidgets.QPushButton("Sanity Check")
        ...
        line_ctrl_row.addWidget(self.btn_sanity_check)
        ...
        self.btn_sanity_check.clicked.connect(self.run_sanity_checks)
    ...
    def _get_cine_frame(self, cine_key: str, t: int) -> np.ndarray:
        frame = self._get_cine_frame_raw(cine_key, t)
        return self._apply_display_transform_to_image(frame)
    ...
    def _cine_axis_permutation(self) -> Tuple[int, int, int]:
        mapping = {
            "X Y Z": (0, 1, 2),
            "X Z Y": (0, 2, 1),
            "Y X Z": (1, 0, 2),
    ...
    def _cine_axis_flips(self) -> np.ndarray:
        return np.array(
            [
    ...
    def _display_axis_perm2d(self) -> Tuple[int, int]:
        perm = list(self._cine_axis_permutation())
        if perm[0] == 2:
            perm[0] = 0 if perm[1] != 0 else 1
        if perm[1] == 2:
            perm[1] = 1 if perm[0] != 1 else 0
        return perm[0], perm[1]

    def _apply_display_transform_to_image(self, img: np.ndarray) -> np.ndarray:
        perm0, perm1 = self._display_axis_perm2d()
        out = img
        if perm0 == 1 and perm1 == 0:
            out = out.T
        flips = self._cine_axis_flips()
        if flips[0] < 0:
            out = np.flipud(out)
        if flips[1] < 0:
            out = np.fliplr(out)
    ...
    def _transform_line_to_display(self, pts_raw: np.ndarray, H_raw: int, W_raw: int) -> np.ndarray:
        ...
        rows_disp = rows_raw if perm0 == 0 else cols_raw
        cols_disp = rows_raw if perm1 == 0 else cols_raw

        if flips[0] < 0:
            size = H_raw if perm0 == 0 else W_raw
            rows_disp = (size - 1) - rows_disp
        if flips[1] < 0:
            size = H_raw if perm1 == 0 else W_raw
            cols_disp = (size - 1) - cols_disp

        return np.column_stack([cols_disp, rows_disp])

    def _inverse_transform_line_from_display(self, pts_disp: np.ndarray, H_raw: int, W_raw: int) -> np.ndarray:
        pts = np.array(pts_disp, dtype=np.float64)
        rows = pts[:, 1]
        cols = pts[:, 0]
        perm0, perm1 = self._display_axis_perm2d()
        flips = self._cine_axis_flips()
        ...
        rows_raw = np.zeros_like(rows)
        cols_raw = np.zeros_like(cols)
        if perm0 == 0:
            rows_raw = rows
        else:
            cols_raw = rows
        ...
        return np.column_stack([cols_raw, rows_raw])
    ...
    def _log(self, message: str):
        self.memo.appendPlainText(message)

    def run_sanity_checks(self):
        self._log("=== Sanity Check START ===")
        geom = self.pack.geom
        if geom.iop is not None and geom.pixel_spacing is not None:
            row_vec = geom.iop[:3]
            col_vec = geom.iop[3:6]
            normal_vec = np.cross(row_vec, col_vec)
            nn = np.linalg.norm(normal_vec)
            normal_vec = normal_vec / (nn if nn > 0 else 1e-12)
            if geom.ipp0 is not None:
                self._log(f"4D IPP0: {np.array2string(geom.ipp0, precision=6, separator=',')}")
            else:
                self._log("4D IPP0: missing")
            self._log(f"4D IOP row: {np.array2string(row_vec, precision=6, separator=',')}")
            self._log(f"4D IOP col: {np.array2string(col_vec, precision=6, separator=',')}")
            self._log(f"4D normal: {np.array2string(normal_vec, precision=6, separator=',')}")
            self._log(
                f"4D PixelSpacing (row,col): {np.array2string(geom.pixel_spacing, precision=6, separator=',')}"
            )
            if geom.slice_positions is not None and geom.slice_positions.size > 1:
                diffs = np.diff(np.sort(geom.slice_positions))
                diffs = diffs[np.isfinite(diffs)]
                if diffs.size:
                    self._log(f"4D slice spacing (median): {float(np.median(diffs)):.6f}")
                self._log(
                    "4D slice positions (min,max): "
                    f"{float(np.min(geom.slice_positions)):.6f}, {float(np.max(geom.slice_positions)):.6f}"
                )
                if geom.slice_order is not None:
                    self._log(
                        f"4D slice order head/tail: {geom.slice_order[:3].tolist()} ... "
                        f"{geom.slice_order[-3:].tolist()}"
                    )
        else:
            self._log("4D geometry: missing IOP/PixelSpacing metadata.")

        if self.active_cine_key in self.pack.cine_planes:
            cine_geom = self._get_cine_geom_raw(self.active_cine_key)
            row_vec = cine_geom.iop[:3]
            col_vec = cine_geom.iop[3:6]
            normal_vec = np.cross(row_vec, col_vec)
            nn = np.linalg.norm(normal_vec)
            normal_vec = normal_vec / (nn if nn > 0 else 1e-12)
            self._log(f"{self.active_cine_key} IPP: {np.array2string(cine_geom.ipp, precision=6, separator=',')}")
            self._log(f"{self.active_cine_key} IOP row: {np.array2string(row_vec, precision=6, separator=',')}")
            self._log(f"{self.active_cine_key} IOP col: {np.array2string(col_vec, precision=6, separator=',')}")
            self._log(f"{self.active_cine_key} normal: {np.array2string(normal_vec, precision=6, separator=',')}")
            self._log(
                f"{self.active_cine_key} PixelSpacing (row,col): "
                f"{np.array2string(cine_geom.ps, precision=6, separator=',')}"
            )

        Ny, Nx, Nz = self.pack.pcmra.shape[:3]
        center = np.array([[Ny / 2.0, Nx / 2.0, Nz / 2.0]], dtype=np.float64)
        xyz = self._voxel_to_patient(center)
        ijk_back = self._patient_to_voxel(xyz)
        err = np.linalg.norm(ijk_back - center)
        self._log(f"voxel->patient->voxel error: {err:.6e} vox")
        ...
        if self.active_cine_key in self.pack.cine_planes:
            ...
            self._log(
                f"cine line endpoints voxel coords: {np.array2string(vox, precision=3, separator=',')}"
            )
            if not inside:
                self._log("WARNING: cine line endpoints outside PCMRA bounds (coordinate mismatch).")
            else:
                self._log("cine line endpoints inside PCMRA bounds.")

        self._log("=== Sanity Check END ===")
    ...
    def on_cine_line_changed_live(self, cine_key: str):
        ...
        pts_abs = self._inverse_transform_line_from_display(pts_abs, H_raw, W_raw)
    ...
    def set_phase(self, t: int):
        ...
        try:
            ...
            pts_abs = self._transform_line_to_display(pts_abs, H_raw, W_raw)
            img = self._get_cine_frame(self.active_cine_key, t)
            H_disp, W_disp = img.shape
            self._apply_line_abs_to_roi(self.cine_line_rois[self.active_cine_key], pts_abs, H_disp, W_disp)
        finally:
            self._syncing_cine_line = False
